  ...
  const purchasePlan = async (planId) => {
    try {
      ...
      if (data.success) {
        ...
        const payUrl = `/payment?orderId=${data.orderId}&upi=${encodeURIComponent(
          data.upiLink
        )}`;

        window.location.href = `/payment?orderId=${data.orderId}&upi=${encodeURIComponent(data.upiLink)}`;
          ...
          >
            <div className="flex-1">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                {plan.name || plan._id.toUpperCase()}
              </h3>
